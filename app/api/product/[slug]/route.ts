...
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      // Slug ile eşleştir veya ID ile eşleştir
      if (product.isActive && product.name && product.imageUrl) {
        if (product.slug === slug || product.id === slug || product.variantId === slug) {
    ...
    if (!foundProduct) {
      return NextResponse.json({ error: "Product not found", product: null }, { status: 404 })
    }

    return NextResponse.json({ product: foundProduct })
  } catch (error) {
    console.error("Product fetch error:", error)
    return NextResponse.json({ error: "Failed to fetch product", product: null }, { status: 500 })
  }
}
